...
`드래그 없이 ctrl + x` | 해당 줄 잘라내기
`드래그 없이 ctrl + c` | 해당 줄 복사하기
`alt + Down` | 행을 아래로 이동
`alt + Up` | 행을 위로 이동
`ctrl + Enter` | 아랫줄에 행 삽입 (줄 중간에 있어도 뒷 내용 무시하고 아랫줄로 넘어감)
`ctrl + shift + Enter` | 윗줄에 행 삽입
`ctrl + alt + Down(방향키)` | 커서를 아래로 확장
`ctrl + alt + Up` | 커서를 위로 확장
`ctrl + d` | 현재 선택 항목과 같은 다음 선택 찾기
`ctrl + shift +l` | 현재 선택 항목과 일치하는 것을 모두 선택
`ctrl + ]` | 라인 들여쓰기
`ctrl + [` | 라인 내어쓰기(커서가 라인 중간에 있어도)
`ctrl + /` | 주석 토글
